...
describe("worker instance ownership", () => {
	...
	it("preserves a failed create when reconciliation confirms the instance is absent", async () => {
		...
		await expect(withOwnedInstance(runtime, "worker-2", use)).rejects.toBe(primary);
		expect(use).not.toHaveBeenCalled();
		expect(runtime.destroyInstance).toHaveBeenCalledWith("worker-2");
	});
	...
	it("destroys an instance committed before its create response was lost", async () => {
		...
		await expect(withOwnedInstance(runtime, "worker-3", async () => undefined)).rejects.toBe(
			primary,
		);
	...
	it("reports cleanup failure without hiding the uncertain create failure", async () => {
		...
		const failure = await withOwnedInstance(runtime, "worker-4", async () => undefined).catch(
			(cause: unknown) => cause,
		);
