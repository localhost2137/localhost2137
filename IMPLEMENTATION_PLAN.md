...
The literal plugin ID and state/config binding happen once per plugin module.
Operations remain standalone descriptor values, and `definePlugin` requires its
literal `id` plus state/config context to match every operation descriptor. The
ID prevents structurally identical plugins from exchanging descriptors by
accident. This is intentionally more explicit than trying to infer an earlier
callback context backwards from a later plugin object, while avoiding
per-operation generics or a plugin builder DSL.
...
- service keys `_`, `clock`, `destroy`, `env`, `idle`, `reset`, and `seed` are
  reserved for instance capabilities; all other names match
  `^[a-z][a-z0-9-]*$`;
- operation key `connection` is reserved for generated connection metadata;
- the TypeScript authoring boundary rejects both reserved-name sets, and Phase
  1 config resolution and operation registration enforce the same sets for
  runtime-loaded or otherwise untyped definitions;
...
`InstanceHandle`: lifecycle methods, `idle()`, clock access, and merged env do
...
The external v0.1 instance client exposes only asynchronous
`instance.clock.status()`, returning `{ mode, now }` with `now` encoded as RFC
3339. The lease-scoped scenario facade has no clock capability. Client clock
advancement does not enter the public type surface until v0.2.
...
In v0.1 the root `clock` capability is read-only: `clock.status()` asynchronously
returns the current mode and RFC 3339 instant. It has no `advance` method.
