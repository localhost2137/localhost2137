...
- For parallel workers, let one owner process hold the test runtime. Give its loopback URL and
  runtime-wide bearer token only to trusted test workers through private harness context—never to
  application or browser code, logs, artifacts, or an untrusted worker. Each worker must select a
  unique instance ID before create and use an ownership helper that never uses or destroys an ID
  rejected by the server as a conflict. That helper is a cleanup and collision-safety convention,
  not authorization isolation. Instances isolate emulator state, not shared plugin config or
  application callback URLs; inspect the plugin's callback-routing semantics before parallel event
  or webhook tests.
