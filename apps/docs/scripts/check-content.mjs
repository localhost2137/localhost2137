...
assert(security?.includes("`localhost env` renders plugin connection values"));
assert(security?.includes("`localhost run` overlays those values on the"));
assert(security?.includes("child's inherited environment"));
assert(security?.includes("Neither command injects the control token"));
assert(security?.includes("`run` does not\nsanitize inherited variables"));
assert(!security?.includes("they supply only plugin connection values"));
assert(security?.includes("It does not contain a same-user process"));
