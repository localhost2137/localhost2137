...
Slack-shaped route. Replacing `fetch(...users.list)` with a `listUsers` control operation would no
longer prove request routing, bearer authentication, response shape, or the application adapter.
