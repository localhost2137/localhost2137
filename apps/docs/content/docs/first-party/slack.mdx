...
all instances of this mount deliver to the same URL. Event IDs and workspace IDs are stable inside
one instance but repeatable across instances; they do not route a shared receiver by themselves.
Serialize such scenarios, use application data that genuinely distinguishes them, or configure
separate runtimes. localhost2137 does not add an instance header.
...
| `users.list` | Returns the bot and local users in creation order; accepts `limit` and `cursor`. |
...
| `createChannel` | `name`; normalizes it to lowercase, creates one public channel, and joins the bot. Valid names contain 1–80 letters, numbers, `_`, or `-`. |
...
trigger an inbound event, but those users cannot authenticate to the Web API. IDs and timestamps are
deterministic local values. Treat provider behavior outside the tables above as unsupported and keep
a provider-facing check when it matters to the application.
