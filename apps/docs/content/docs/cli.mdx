...
LOCALHOST_INSTANCE=review pnpm exec localhost describe slack --json
pnpm exec localhost describe slack --instance dev --json
```
...
```sh
pnpm exec localhost exec slack create-user --name Grace --admin=false --json
...
pnpm exec localhost seed --instance dev
